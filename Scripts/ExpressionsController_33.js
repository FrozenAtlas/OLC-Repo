...
// @input Component.RenderMeshVisual blendShapesComponent            {"label":"Blendshapes Component"}
...
var expressionNames = new Array(),
    ...
    blendShapeNames = new Array(),
    isBlendShapesChecked = false;
...
if (!script.blendShapesComponent) {
...
function checkBlendShapes() {
    for (var i = 0; i < expressionNames.length; i++) {
        ...
        var blendShapeName = getBlendShapeName(name);
        blendShapeNames.push(blendShapeName);
        
        if (script[name] && !script.blendShapesComponent.hasBlendShapeWeight(blendShapeName)) {
            print("ExpressionController, ERROR: No blendshape with name " + blendShapeName);
        }
    }
}

function getBlendShapeName(expressionName) {
    var name = expressionName;
    
    var mappedName = script[name + "Rename"];
...
function setBlendShapes() {    
    for (var i = 0; i < expressionNames.length; i++) {
        ...
        if (script[name] == null || typeof script[name] === "undefined") {
            continue;
        }
        
        var blendShapeName = blendShapeNames[i];
        
        if (blendShapeName && script.blendShapesComponent.hasBlendShapeWeight(blendShapeName)) {
            setExpressionWeight(script.blendShapesComponent, blendShapeName, i, script[name + "Scale"]);
        }
    }  
}

function setExpressionWeight(blendComponent, shapeName, expressionIndex, scale) {
    const appliedScale = scale || 1.0;
    const w = expressionWeights[expressionIndex] * appliedScale;
    blendComponent.setBlendShapeWeight(shapeName, w);
}
...
function onUpdate() {
    ...
    if (!isBlendShapesChecked && script.blendShapesComponent) {
        checkBlendShapes();
