...
TWEEN.Tween.prototype.resume = function(tweenName){
    if ( !this._isPaused )
    {
        print ( "Tween Manager: " + tweenName + " has not been paused. Did you mean to call startTween this Tween instead?");
        return;
    }
...
TWEEN.Tween.prototype.pause = function(tweenName){
    ...
    if ( this._isPaused )
    {
        print ( "Tween Manager: Warning, " + tweenName + ", has already been paused.");
        return;
    }
...
function resumeTween( tweenObject, tweenName )
{
    ...
    if ( tweenScriptComponent )
    {
        ...
        else if( tweenScriptComponent.api.tween )
        {
            if ( Array.isArray(tweenScriptComponent.api.tween) )
            {
                ...
                {
                    tweenScriptComponent.api.tween[i].resume(tweenName);
                }
            }
            else
            {
        ...
        {
            print( "Tween Manager: Warning, trying to resume " + tweenName + ", which hasn't been initialized" );
        }
    }
    else
    {
        print( "Tween Manager: Trying to resume " + tweenName + ", which does not exist. Ensure that the Tween Type has been initialized and started.");
    }
}

// Pause a tween that is currently playing
function pauseTween( tweenObject, tweenName )
{
    var tweenScriptComponent = findTween( tweenObject, tweenName );
    if ( tweenScriptComponent )
    {
        ...
        else if( tweenScriptComponent.api.tween )
        {
            if ( Array.isArray(tweenScriptComponent.api.tween) )
            {
                for ( var i = 0; i < tweenScriptComponent.api.tween.length; i++)
                {
                    tweenScriptComponent.api.tween[i].pause(tweenName);
                }
            }
            else
            {
        ...
        {
            print( "Tween Manager: Warning, trying to pause " + tweenName + ", which hasn't been initialized" );
        }
    }
    else
    {
        print( "Tween Manager: Trying to pause " + tweenName + ", which does not exist. Ensure that the Tween Type has been initialized and started.");
    }
}
...
function startTween( tweenObject, tweenName, completeCallback, startCallback, stopCallback )
{
    ...
    if( tweenScriptComponent )
    {
        print( "Tween Manager: Starting " + tweenName );
        ...
        if ( tweenScriptComponent.api.tweenType == "chain" )
        {
            ...
            if( stopCallback )
            {
                for ( var i = 0; i < tweenScriptComponent.api.allTweens.length; i++ )
                {
                    var currentTween = tweenScriptComponent.api.allTweens[i];
...
function stopTween( tweenObject, tweenName )
{
    ...
    if( tweenScriptComponent )
    {
        print( "Tween Manager: Stopping " + tweenName );
        if ( tweenScriptComponent.api.tweenType == "chain")
        {
            if ( tweenScriptComponent.api.playAll )
            {
            ...
            else if( tweenScriptComponent.api.tween )
            {
                if ( Array.isArray(tweenScriptComponent.api.tween) )
                {
                    for ( var i = 0; i < tweenScriptComponent.api.tween.length; i++)
                    {
                        tweenScriptComponent.api.tween[i].stop();
                    }
                }
                else
                {
            ...
            {
                print( "Tween Manager: Warning, trying to stop " + tweenName + ", which hasn't been started" );
            }
...
function setStartValue( tweenObject, tweenName, startValue )
{
    ...
    if ( tweenScriptComponent )
    {
        ...
        {
            print ( "Tween Manager: You cannot manually set the start value of " + tweenName );
        }
    }
}
...
function setEndValue( tweenObject, tweenName, endValue )
{
    ...
    if ( tweenScriptComponent )
    {
        ...
        {
            print ( "Tween Manager: You cannot manually set the end value of " + tweenName );
        }
    }
}
...
function resetObject( tweenObject, tweenName )
{
    ...
    if( tweenScriptComponent )
    {
        print( "Tween Manager: Resetting Object " + tweenName );
...
function findTween( tweenObject, tweenName )
{
    for( var i = 0; i < tweenObject.getComponentCount( "Component.ScriptComponent"); i++ )
    {
        ...
        {
            print( "Tween Manager: Tween type hasn't initialized. Ensure that " + tweenName + " is on \"Lens Turn On\" and that Tween Manager is at the top of the Objects Panel." );
            return;
        }
    }

    print( "Tween Manager: Tween, " + tweenName + ", is not found. Ensure that " + tweenName + " is on \"Lens Turn On\" and that Tween Manager is at the top of the Objects Panel.");
    return;
}
...
function findTweenRecursive( tweenObject, tweenName )
{
    ...
    for ( var i = 0; i < tweenObject.getChildrenCount(); i++ )
    {
        var result = findTweenRecursive( tweenObject.getChild(i), tweenName );
        if ( result )
        {
            return result;
        }
            ...
            if( tweenName == scriptComponent.api.tweenName )
            {
                if( scriptComponent.api.tweenType == "value" )
                {
                    if ( scriptComponent.api.tween )
                    {
                        ...
                        {
                            print ( "Tween Manager: Tween Value, " + tweenName + ", is not currently playing. Ensure that it has been started by either calling its startTween() function or by setting it to Play Automatically.")
                        }
                    }
                    else
                    {
                        print ( "Tween Manager: Tween Value, " + tweenName + ", has not been set up. Ensure that this Tween Value is ordered before every other script that uses it in the Objects Panel and Inspector. Try initializing it in the Initialized event and scripting in the Lens Turn On event.");
                    }
